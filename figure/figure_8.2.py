...
绘制多模型对比
"""
def draw_mag_magerr(k):
    df = pd.read_csv("./figure_8/test_{}.csv".format(k))[:]
    arr = ["double","triple","quadruple","all"]
    boxplot_func(df, "mag", "mag_mape", "class",
                 [18, 22], [-0.6, 4], [-0.2, 2.5],
                 "not",
                 ["CAT-deblender","Sextractor"],
                 "{} blended galaxies magnitude".format(arr[k]), "Magnitude error $\Delta{mag}$", "P($\Delta_{mag}}$)", "median", palette=["purple","blue"],png="1_{}".format(k))
for name in [
            1,2,3,4
        ]:
    draw_mag_magerr(name)
