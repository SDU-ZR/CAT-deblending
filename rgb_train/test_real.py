...
from new_model_rgb_train import *
class BigDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        memmap_index = bisect(self.start_indices, index) - 1
        index_in_memmap = index - self.start_indices[memmap_index]
        img = self.data_memmaps[memmap_index][index_in_memmap,:, :, :]

        img = np.squeeze(img)
        # label_down = np.squeeze(label_down)
...
class Trainer:
    ...
    def __init__(self,args):
        ...
        if self.args.resume:
            if not os.path.exists(self.args.save_path):
                ...
                self.epoch = param_dict_up["epoch"]
                self.lr = param_dict_up["lr"]
                self.gnet_up.load_state_dict(param_dict_up["gnet_dict"])
                self.best_psnr = param_dict_up["best_psnr"]
                self.best_ssim = param_dict_up["best_ssim"]
    ...
    def val(self, epoch):
        ...
        psnrs = []
        ssims = []
        data = np.zeros((len(self.val_loader),3,128,128,3))
        print("Test start...")
        with torch.no_grad():
            for i, (index, img) in enumerate(self.val_loader):
                img = img.float()
                ...
                fake_up_img = convert_image(fake_up_img)
                ...
                data[i,0,:,:,:] = img
                data[i,2,:,:,:] = fake_up_img
                data[i,1,:,:,:] = fake_down_img
                fig = plt.figure()
                # ssim_x = np.around(ssim_x, decimals=2)
                # ssim_y = np.around(ssim_y, decimals=2)
                ...
                # ax1 = fig.add_subplot(gs[0, 0])
                # ax2 = fig.add_subplot(gs[1, 0])
                ax3 = fig.add_subplot(gs[0:3, 0:2])
                ...
                ax3.imshow(img)
                ax3.text(1.3, 4.4, r'Blended', color='#FFFFFF')
                ax4.imshow(fake_down_img)
                ax4.text(3., 10., r'Deblended one', color='#FFFFFF')
                # ax4.text(3., 75., str(round(ssim_x,2))+"dB", color='#FFFFFF')  #
                ax5.imshow(fake_up_img)
                ax5.text(3., 10., r'Deblended double', color='#FFFFFF')
                # ax5.text(3., 75., "ssim :"+str(round(1,2)), color='#FFFFFF')  #

                plt.tight_layout(pad=0)
                plt.subplots_adjust(wspace=0.06, hspace=-0.42)
                # fig.savefig("../figure/figure_9/catalog_deblend_real_galaxy/{}".format(files[i]))
                print("-------------已经完成第{}个测试------------------".format(i))
                # if i>=0:
                #     break
            np.save("./predict_data/catalog_deblend_real_galaxy.npy",data)
            return ssims, psnrs
...
if __name__ == '__main__':
    ...
    parser.add_argument("--val_data_path", default="./predict_data/catalog_real_blend_galaxy.npy", type=str)
    parser.add_argument("--resume", default=True, type=bool)
    parser.add_argument("--num_epochs", default=100, type=int)
    parser.add_argument("--save_path", default="./weight/decals_up_weight1.pt", type=str)
    parser.add_argument("--save_path1", default="./weight/decals_up_weight1.pt", type=str)
